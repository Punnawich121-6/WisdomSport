...
const info4 = document.getElementById("tab-4")

async function getBasketball(db) {
    const basCol = collection(db, "Volleyballskill")
...
function showData(bas) {
    ...
    skill.push(SkillDoc);
    info1.innerHTML = skill[1];
    info2.innerHTML = skill[2];
    info3.innerHTML = skill[0];
    info4.innerHTML = skill[3];
}
